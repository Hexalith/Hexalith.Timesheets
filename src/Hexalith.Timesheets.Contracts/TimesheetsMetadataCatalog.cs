...
public static class TimesheetsMetadataCatalog
{
    ...
    [
        ...
        new(
            ...
            TimesheetsCompositionPattern.FrontComposerProjectionView,
            [
                new("projectFilter", "Project filter", "ProjectReference", false, "Project references remain stable IDs; labels hydrate after authorization."),
                new("contributorFilter", "Contributor filter", "PartyReference", false, "Preserved during drill-in and back navigation."),
                new("activityTypeFilter", "Activity Type filter", nameof(ActivityTypeId), false),
                new("periodFilter", "Period filter", "TenantLocalPeriod", false, "Tenant-local period or date range."),
                new("billableFilter", "Billable filter", nameof(BillableState), false),
                new("approvalStateFilter", "Approval state filter", nameof(TimeEntryApprovalState), false),
                new("contributorCategoryFilter", "Contributor category filter", nameof(ContributorCategory), false),
                new("aiAgentFilter", "AI agent filter", "PartyReference", false, "Automated agent Party filter for AI effort rows."),
                new("aiMetricAvailabilityFilter", "AI metric availability filter", nameof(AiMetricAvailability), false),
                new("aiTokenAvailabilityFilter", "AI token availability filter", nameof(AiTokenMetricAvailability), false, "Unavailable and Not reported by provider are displayed as text."),
                new("aiSourceCategoryFilter", "AI source category filter", nameof(AiEffortMetricSourceCategory), false),
                new("currentRowsOnly", "Current rows only", "Boolean", true, "Superseded evidence is hidden unless explicitly requested."),
                new("includeSupersededRows", "Include superseded rows", "Boolean", false, "Shows superseded evidence only for reconciliation."),
                new("projectionFreshnessFilter", "Projection freshness filter", nameof(ProjectionFreshnessState), false),
                new("target", "Project reference", "ProjectReference", true),
                new("tenantLocalPeriodKey", "Period", "TenantLocalPeriod", true),
                new("contributor", "Contributor", "PartyReference", true),
                new("activityType", "Activity Type", nameof(ActivityTypeId), true),
                new("billableState", "Billable state", nameof(BillableState), true),
                new("approvalState", "Approval state", nameof(TimeEntryApprovalState), true),
                new("contributorCategory", "Contributor category", nameof(ContributorCategory), true),
                new("actualMinutes", "Actual minutes", "WholeMinutes", true),
                new("aiWallClockDurationMilliseconds", "AI wall-clock execution", "Milliseconds", false, "AI wall-clock execution stays separate from human and external minutes."),
                new("aiModelRuntimeMilliseconds", "AI model/tool runtime", "Milliseconds", false),
                new("aiBillableEffortMinutes", "AI billable effort", "WholeMinutes", false, "AI billable effort stays separate from actual minutes and provider token counts."),
                new("aiProviderInputTokenCount", "Provider input tokens", "Integer", false, "Unavailable token counts display Unavailable or Not reported by provider, never zero unless reported as zero."),
                new("aiProviderOutputTokenCount", "Provider output tokens", "Integer", false, "Unavailable token counts display Unavailable or Not reported by provider, never zero unless reported as zero."),
                new("aiProviderTotalTokenCount", "Provider total tokens", "Integer", false, "Provider-reported token count only."),
                new("aiMetricAvailability", "AI metric availability", nameof(AiMetricAvailability), true),
                new("aiTokenAvailability", "AI token availability", nameof(AiTokenMetricAvailability), true, "Unavailable or Not reported by provider."),
                new("aiMetricSourceCategory", "AI source category", nameof(AiEffortMetricSourceCategory), false),
                new("aiMetricSourceMetadata", "AI metric source metadata", nameof(AiEffortMetricSourceMetadata), false, "Compact provider, tool, or work execution reference only."),
                new("sourceRowCount", "Source rows", "Integer", true),
                new("correctionCount", "Corrections", "Integer", true),
                new("supersededCount", "Superseded rows", "Integer", true),
                new("referenceState", "Reference state", nameof(ActualTimeReferenceState), true),
                new("displayHydration", "Display hydration", nameof(TimeEntryDisplayHydration), true, "Labels are hydrated only after row authorization succeeds."),
                new("projectionFreshness", "Projection freshness", nameof(ProjectionFreshnessState), true)
            ],
            [
                new("query-project-actual-time-report", "Query Project report", "Timesheets.QueryProjectActualTimeReport"),
                new("open-work-actual-time-report", "Open Work report", "Timesheets.QueryWorkActualTimeReport"),
                new("drill-into-evidence", "Open evidence", "Timesheets.ReadTimeEntryEvidence")
            ],
            [
                new("approval", "Approval", nameof(TimeEntryApprovalState)),
                new("billable", "Billable", nameof(BillableState)),
                new("contributor", "Contributor", nameof(ContributorCategory)),
                new("report-row", "Report row", nameof(ActualTimeReportRowState)),
                new("reference-state", "Reference state", nameof(ActualTimeReferenceState)),
                new("hydration-state", "Hydration state", nameof(DisplayHydrationState)),
                new("ai-metrics", "AI metric availability", nameof(AiMetricAvailability)),
                new("ai-tokens", "AI token availability", nameof(AiTokenMetricAvailability)),
                new("ai-source", "AI metric source category", nameof(AiEffortMetricSourceCategory)),
        ...
        new(
            ...
            "Work Actual-Time Report",
            "reporting",
            TimesheetsSurfaceKind.Projection,
            TimesheetsCompositionPattern.FrontComposerProjectionView,
            [
                new("workFilter", "Work filter", "WorkReference", false, "Work references remain stable IDs; labels hydrate after authorization."),
                new("contributorFilter", "Contributor filter", "PartyReference", false, "Preserved during drill-in and back navigation."),
                new("activityTypeFilter", "Activity Type filter", nameof(ActivityTypeId), false),
                new("periodFilter", "Period filter", "TenantLocalPeriod", false, "Tenant-local period or date range."),
                new("billableFilter", "Billable filter", nameof(BillableState), false),
                ...
                new("includeSupersededRows", "Include superseded rows", "Boolean", false, "Shows superseded evidence only for reconciliation."),
                new("projectionFreshnessFilter", "Projection freshness filter", nameof(ProjectionFreshnessState), false),
                new("target", "Work reference", "WorkReference", true),
                new("tenantLocalPeriodKey", "Period", "TenantLocalPeriod", true),
                new("contributor", "Contributor", "PartyReference", true),
                new("activityType", "Activity Type", nameof(ActivityTypeId), true),
                new("billableState", "Billable state", nameof(BillableState), true),
                ...
                new("referenceState", "Reference state", nameof(ActualTimeReferenceState), true),
                new("displayHydration", "Display hydration", nameof(TimeEntryDisplayHydration), true, "Labels are hydrated only after row authorization succeeds."),
                new("plannedEffortAvailability", "Planned effort availability", nameof(WorkPlannedEffortAvailability), true, "Works is the source when planned values are supplied."),
                new("plannedEstimated", "Works estimated effort", "Decimal", false),
                new("plannedDone", "Works done effort", "Decimal", false),
                new("plannedRemaining", "Works remaining effort", "Decimal", false),
                new("plannedUnit", "Works effort unit", "String", false),
                new("plannedSourceReferenceState", "Works source reference state", nameof(ActualTimeReferenceState), true),
                new("plannedSourceFreshness", "Works source freshness", nameof(ProjectionFreshnessState), true),
                new("projectionFreshness", "Projection freshness", nameof(ProjectionFreshnessState), true)
            ],
            [
                new("query-work-actual-time-report", "Query Work report", "Timesheets.QueryWorkActualTimeReport"),
                new("open-project-actual-time-report", "Open Project report", "Timesheets.QueryProjectActualTimeReport"),
                new("drill-into-evidence", "Open evidence", "Timesheets.ReadTimeEntryEvidence")
            ],
            [
                new("approval", "Approval", nameof(TimeEntryApprovalState)),
                ...
                new("reference-state", "Reference state", nameof(ActualTimeReferenceState)),
                new("hydration-state", "Hydration state", nameof(DisplayHydrationState)),
                new("planned-effort", "Planned effort availability", nameof(WorkPlannedEffortAvailability)),
                new("ai-metrics", "AI metric availability", nameof(AiMetricAvailability)),
                new("ai-tokens", "AI token availability", nameof(AiTokenMetricAvailability)),
                new("ai-source", "AI metric source category", nameof(AiEffortMetricSourceCategory)),
                new("projection-freshness", "Projection freshness", nameof(ProjectionFreshnessState))
