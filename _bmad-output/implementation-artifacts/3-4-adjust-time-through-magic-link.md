...
## 2026-09-16 Loader and Activity Type Ownership Clarification

The concrete EventStore-backed loader and live token-index/catalog projection handlers now exist and are wired, resolving the historical unchecked concrete-loader follow-up above. Adjustment now accepts only a tenant-owned Activity Type while preserving Project and Work targets. Newly emitted adjustment events mirror the server-resolved tenant scope into both previous and adjusted snapshots, so aggregate and projection replay retain the authorized decision; project-owned or inconsistent folded evidence fails closed without capability use or a Time Entry event.
